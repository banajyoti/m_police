    ...
    public function register(Request $request)
    {
        if ($request->isMethod('GET')) {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'm_sport' => 'required_if:permanent_residence,1',
            ...
            if ($request->ex_ser == '1') {
                // Default age ranges based on category_id
                $ageRanges = [
                    '1' => ['min' => "1997-01-20", 'max' => "2007-01-20", 'message' => 'Age must be between 18-28 years'],
                    '3' => ['min' => "1992-01-20", 'max' => "2007-01-20", 'message' => 'Age must be between 18-33 years'],
                    '4' => ['min' => "1992-01-20", 'max' => "2007-01-20", 'message' => 'Age must be between 18-33 years'],
                    '2' => ['min' => "1994-01-20", 'max' => "2007-01-20", 'message' => 'Age must be between 18-31 years']
                ];

                // Check if the category_id exists in the predefined ranges
                if (isset($ageRanges[$request->category_id])) {
                    $minAge = $ageRanges[$request->category_id]['min'];
                    $maxAge = $ageRanges[$request->category_id]['max'];
                    $ageMessage = $ageRanges[$request->category_id]['message'];

                    // Adjust minAge if 'calculated_age' is provided
                    if (isset($request->calculated_age)) {
                        ...
                        if (count($matches) === 4) {
                            ...
                            // Calculate the adjusted minAge based on the calculated age
                    ...
                    if ($request->dob < $minAge || $request->dob > $maxAge) {
                        return response()->json(['status' => 'error', 'errors' => ['dob' => [$ageMessage]]], 422);
                    }
                }
                ...
                if ($request->category_id == '1') {
                    if ($request->dob < "1997-01-20" || $request->dob > "2007-01-20") {
                        return response()->json(['status' => 'error', 'errors' => ['dob' => ['Age must be between 18-28 years']]], 422);
                    }
                } elseif ($request->category_id == '3' || $request->category_id == '4') {
                    if ($request->dob < "1992-01-20" || $request->dob > "2007-01-20") {
                        return response()->json(['status' => 'error', 'errors' => ['dob' => ['Age must be between 18-33 years']]], 422);
                    }
                } elseif ($request->category_id == '2') {
                    if ($request->dob < "1994-01-20" || $request->dob > "2007-01-20") {
                        return response()->json(['status' => 'error', 'errors' => ['dob' => ['Age must be between 18-31 years']]], 422);
                    }
                }
            }
            //dd($minAge,$request->calculated_age,$request->dob,$request->total_ex);
